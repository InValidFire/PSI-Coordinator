// This file contains a list of the current and available classids/codes at GGC

export const available_class_codes_and_names =
    [
    "ITEC 2140 - Programming Fundamentals",
    "ITEC 2150 - Intermediate Programming",
    "ACCT 2102 - Introduction to Managerial Accounting",
    "BIOL 1107K - Principles of Biology I with Laboratory",
    "BIOL 1108K - Principles of Biology II with Laboratory",
    "CHEM 1211K - Principles of Chemistry I with Laboratory",
    "CHEM 1212K - Principles of Chemistry II with Laboratory",
    "CHEM 2211K - Organic Chemistry I with Laboratory",
    "ENGL 1101 - English Composition I",
    "ENGL 1102 - English Composition II"
    ]